(function(){
  ...
  var mapBtn = $('#js_open_map'),
      mapElmWrapper = $('.google_map'),
      mapElm = $('#js_google_map'),
      inputLatitude= $('#w-latitude'),
      inputLongitude= $('#w-longitude');

  mapBtn.attr('disabled', true);

  var x = 1;
  let opacityCss = mapElmWrapper.css( "opacity" );
  mapBtn.click(function() {
    //if(opacityCss !== 1) mapElmWrapper.css( "opacity",1 );
    mapElm.slideToggle(200);
    if ((x % 2) != 0) {
      $(this).text('Open Map');
      //mapElmWrapper.css( "height", "0" );
      x++;
    }
    else {
      $(this).text('Hide Map');
      //mapElmWrapper.css( "height", "240px" );
      x++;
    }
...
// The user can then click an option to hide, show or delete the markers.
  ...
  function initMap() {
    ...
    mapElement.style.height = 240 + "px";//need to init map correct

    mapBtn.attr('disabled',false);
  }
