...
/* WEBPACK VAR INJECTION */(function(global) {module.exports = global["ko"] = __webpack_require__(77);
...
var CustomInput = __webpack_require__(39);

describe("CustomInput component spec", function () {
	var customInput = void 0;
	...
	it("Sets accepted CSS class", function () {
		expect(customInput.cssClass).toBe('testCssClass');
	});

	describe("'values' prop check", function () {
		it("'values' is defined", function () {
			expect(customInput.data.value).toBeDefined();
		});
		it("'values' is knockout observable", function () {
			expect(ko.isObservable(customInput.data.value)).toBe(true);
		});
	});

	describe("'isDirty' prop check", function () {
		it("'isDirty' is defined", function () {
			expect(customInput.data.isDirty).toBeDefined();
		});

		it("typeof 'isDirty' is knockout computed", function () {
			expect(ko.isComputed(customInput.data.isDirty)).toBe(true);
		});

		it("'isDirty' is truthy", function () {
			expect(ko.unwrap(customInput.data.isDirty)).toBeTruthy();
		});
	});

	describe("'required' prop check", function () {
		it("'required' is defined", function () {
			expect(customInput.data.required).toBeDefined();
		});

		it("typeof 'required' is boolean", function () {
			expect((0, _typeof3.default)(customInput.data.required)).toBe('boolean');
		});

		it("'required' is falsy", function () {
			expect(customInput.data.required).toBeFalsy();
		});
	});

	describe("Model data has correct params and param types", function () {
		it("Model data is not null", function () {
			expect(customInput.data).not.toBe(null);
		});
	});

	describe("Component has correct onFocus handler", function () {
		var component = new CustomInput({
			onFocusMethod: function onFocusMethod(param) {
				return param;
			},
			data: {
				value: ko.observable({}),
		...
		it("Handler is defined", function () {
	...
	describe("Component has correct onEnter key down handler", function () {
		it("Handler is defined", function () {
...
var Clock = module.CountdownClock,
    CDWidget = module.TicketmasterCountdownWidget;

describe("CDWidget", function () {
	var cdwidget = {},
	    widgetRoot = void 0;
