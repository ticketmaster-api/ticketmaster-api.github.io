...
(function($){
    ...
    jQuery.fn.lazySelector = function(options ){
      ...
      var $input = $(this);
      console.log('$input - top' , $input);

      $("body").append($("<script async defer src='https://maps.googleapis.com/maps/api/js?key=AIzaSyBQrJ5ECXDaXVlICIdUBOe8impKIGHDzdA&callback=initMap'></script>"));
      ...
      var loading = function(action) {
          ...
          if(action=="on"){
            spinner.show();
            console.log('spinner.show()',spinner );
          }
          else if(action=="off"){
            spinner.hide();
      ...
      function submitForm( /*optional*/pageNumero){
          ...
          var url = ( Number.isNaN(pageNumero) )
            ? eventUrl+'?apikey='+apikey+'&keyword='+keyword.val()
            : eventUrl+'?apikey='+apikey+'&keyword='+keyword.val()+'&page='+pageNumero;

          //stop load
          if( Number.isNaN(pageNumero) && pageNumero !== 0 && stateConf.loadingFlag==='STOP_LOAD') {
      ...
      var renderResults = function(data, ulElement){
        ...
        function myMap(ltd , lgt) {
          var mapCanvas = document.getElementById("map-canvas");
          var myCenter = new google.maps.LatLng( 55,  43);
